...
- Deployed as Docker container (`output: 'standalone'` in next.config.ts)
- Production Dockerfile uses multi-stage build (deps → build → runner)
- Only `.next/standalone` + `.next/static` + `public/` are in the final image
- Source code is NOT in the production image
- Environment variables injected at runtime, never baked into image
- S3 for file storage (config in `src/api/config/s3.ts`)
- Redis for caching/rate-limiting (config in `src/api/config/redis.ts`)
